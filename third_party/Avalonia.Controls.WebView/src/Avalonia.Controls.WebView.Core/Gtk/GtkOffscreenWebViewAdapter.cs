        ...
        IWebViewAdapterWithOffscreenBuffer, IWebViewAdapterWithOffscreenInput
{
    ...
    private static readonly Func<IntPtr, IntPtr, bool> s_submitGtkEvent = gtk_widget_event;
    ...
    public Task UpdateWriteableBitmap(PixelSize _, FrameChainBase<WriteableBitmap, PixelSize>.IProducer producer)
    {
        ...
        return RunOnGlibThreadAsync(() =>
        {
            ...
            try
            {
                ...
                var sourceStride = gdk_pixbuf_get_rowstride(pixbuf);
    ...
    public bool KeyInput(bool press, PhysicalKey physical, string? keySymbol, KeyModifiers modifiers)
    {
        ...
        return RunOnGlibThread(() =>
        {
            ...
            var eventState = ComposeKeyboardState(keymapState, modifiers);

            using var state = new EventSendState(press ? GdkEventType.GDK_KEY_PRESS : GdkEventType.GDK_KEY_RELEASE, WebViewHandle);
            var ev = state.Event;

            if (!gdk_keymap_translate_keyboard_state(keymap, keycode, eventState, 0,
                    out var keyval, out var effectiveGroup, out _, out _))
            {
                return false;
            }
            ...
            ev->key.state = eventState;
            ev->key.time = 0;

            // Submit synchronously so Avalonia marks the routed key event handled
            // only when GTK/WebKit actually accepted it.
            return state.SendToWidget();
        });
    }
    ...
    internal static GdkModifierType ComposeKeyboardState(
        GdkModifierType liveKeymapState,
        KeyModifiers eventModifiers)
        => (liveKeymapState & GdkModifierType.GDK_KEYBOARD_LOCK_MASK) |
           ToGtk(eventModifiers, null);

    internal static bool SubmitKeyEvent(
        IntPtr widget,
        IntPtr gdkEvent,
        Func<IntPtr, IntPtr, bool> submitToGtk)
        => widget != IntPtr.Zero &&
           gdkEvent != IntPtr.Zero &&
           submitToGtk(widget, gdkEvent);
    ...
    private readonly ref struct EventSendState : IDisposable
    {
        ...
        public bool SendToWidget() => SubmitKeyEvent(_widget, _evPtr, s_submitGtkEvent);
